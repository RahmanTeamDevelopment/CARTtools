...
def perfect_and_cds_matches_synonyms(gene_synonyms, ensembl_data, cart, gene_id, out):

    perfect_matches = []
    identical_cds = []

    if gene_id in gene_synonyms:
        synonyms_in = [g for g in gene_synonyms[gene_id] if g in ensembl_data]
    else:
        synonyms_in = [gene_id] if gene_id in ensembl_data else []
    ...
    for g in synonyms_in:
        ...
        for enst in ensembl_data[g]:
            ...
            if are_perfect_match(cart, enst):
                perfect_matches.append(enst)
                continue

            if have_identical_cds(cart, enst):
                identical_cds.append(enst)

    return perfect_matches, identical_cds


def perfect_and_cds_matches_any_gene(ensembl_tabix_file, cart, gene_id, out):

    perfect_matches = []
    identical_cds = []

    overlapping_ensts = parser.read_ensembl_transcripts(ensembl_tabix_file, cart)
...
def output(out, cart, gene_id, ensts, flag, utr_selection_reason, phase1_difference_type, phase1_decisive_criteria):

    record = [cart.id_, gene_id]
    ...
    if ensts is not None:
        record += [
            ...
            ';'.join([utr.utr_difference(cart, x) for x in ensts]),
            ';'.join([utr.utr_exon_number_difference(cart, x) for x in ensts])
        ]
    else:
        record += ['.'] * 5
